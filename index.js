...
let owners = '1263912333353160838'//ايدي الي هيتحول 
let owner;
const
  updateAdmins = () => {
  owner = db.get("admins")
  owners = '1263912333353160838'}
updateAdmins()
...
client.on('guildCreate', guild => {
  ...
  let textList = ["$Stock" , `${prefix}help` , "BY BST HOST"]
...
`)
    ...
    if (command === "buy") {
      ...
      if (cooldown === true) return message.reply("*انتظر 1 دقيقة وحاول مجددا ** " + "<@" + message.author.id + ">");
      const account = args[0], numOfAcc = args[1] || 1
      
      const data = db.get("accounts")
      let accInDb = data.find(el => el.name === account)
      if (!accInDb) { return message.channel.send("**لايتوفر هاذا المنتج ❌**") }
      ...
      let buys = new Discord.MessageEmbed()
        ...
        .setTimestamp()
        .addField(`**شراء ${account} منتج :**`
          ,
          `
            
            > **-عدد المنتجات :** \`${numOfAcc}\`
        ...
        message.channel.awaitMessages({
          ...
          message.author.send({
            embeds: [new Discord.MessageEmbed()
              ...
              .setTimestamp()
              .setDescription(`**Hi ${message.author.tag} , you have purchased ${numOfAcc} ${account} Products${numOfAcc > 1 ? "s" : ""}:**
> **Total price : \`${tax * numOfAcc}\`**
          **Your Product${numOfAcc > 1 ? "s" : ""}:**
...
`)
  ...
  .setTitle("😊تم ارسال المنتج فالخاص نتمنى منك تقييم الخدمة <#1228121643134488682>")
  .setColor('RED')
  message.channel.send({embeds : [embed]})
          ...
          .catch(err => {
            ...
            console.log(err)
            msg.delete()
            message.reply("**انتهى الوقت لاتقم بالتحويل نحن لسنا سؤولين!!!**")
          })
...
client.login("MTI5MDgxMDQyNTk3MzQwNzc2NA.GS9yLH.EqOoVrTZr0uodo0epAB6K3jSmSi4wXr9ThzpaQ")//token bot
